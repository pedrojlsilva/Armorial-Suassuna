...
void Team::addPlayer(uint8 playerNum) {
    ...
    (void) _playersFatigues.insert(playerNum, 0.0);
    _nPlayers++;
}
...
void Team::delPlayer(uint8 playerNum) {
    ...
    (void) _playersFatigues.remove(playerNum);
    _nPlayers--;
}
...
/*** 'fatigue' function
  ...
  ** Returns:     Nothing
  ***/
void Team::setFatigue(uint8 playerNum, float fatigue) {
    // Handles the lock
    #ifdef GSTHREADSAFE
    QWriteLocker possessionsLocker(_possessionsLock);
    // TODO: Fix fault at this point
    ...
    // Sets the flag
    if (_validPlayers.value(playerNum)) {
        (void) _playersFatigues.insert(playerNum, fatigue);
    }
    else {
        #ifdef GSDEBUGMSG
        cerr << ">> GEARSystem: Team::setFatigue(uint8, float): No such Player #" << int(playerNum);
        cerr << " in Team #" << int(_number) << "(" << _name.toStdString() << ")!!" << endl << flush;
        #endif
    }
}
...
float Team::fatigue(uint8 playerNum) const {
    // Handles the lock
    #ifdef GSTHREADSAFE
    QReadLocker possessionsLocker(_possessionsLock);
    // TODO: Fix fault at this point
    ...
    // Returns the flag
    if (_validPlayers.value(playerNum)) {
        return(_playersFatigues.value(playerNum));
    }
    else {
        #ifdef GSDEBUGMSG
        cerr << ">> GEARSystem: Team::fatigue(uint8): No such Player #" << int(playerNum) << " in Team #";
        cerr << int(_number) << "(" << _name.toStdString() << ")!!" << endl << flush;
        #endif
    }
