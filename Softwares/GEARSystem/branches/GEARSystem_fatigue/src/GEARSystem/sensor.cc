...
/*** 'fatigue' function
  ...
  ** Returns:     Nothing
  ***/
void Sensor::setFatigue(uint8 teamNum, uint8 playerNum, float fatigue) {
    // Sets the flag
    if (isConnected()) {
        try {
            _corbaSensor->setFatigue(teamNum, playerNum, fatigue);
        }

        // Handles CORBA exceptions
        catch (const CORBA::Exception& exception) {
            #ifdef GSDEBUGMSG
            cerr << ">> GEARSystem: Sensor::setFatigue(uint8, uint8, const float): ";
            cerr << "Caught CORBA exception: " << exception._name() << "!!" << endl << flush;
            #endif
        }
    }
