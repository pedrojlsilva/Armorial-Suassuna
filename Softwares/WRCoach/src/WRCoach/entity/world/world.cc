...
#include <WRCoach/entity/world/worldmapupdater.hh>
...
World::World(Controller *ctr, Competitions::Competition competition, Fields::Field *defaultField) : Entity(ENT_WORLD) {
    _ctr = ctr;
    _ourTeam = _opTeam = NULL;
    ...
    _wmUpdater->setDefaultFieldGeometry(_wm, competition);

    // Set self loop time
    this->setLoopFrequency(GlobalConstants::threadLoopFrequency());
...
void World::setTeams(WRTeam *ourTeam, WRTeam *opTeam) {
    _ourTeam = ourTeam;
    _opTeam = opTeam;
}
...
void World::initialization() {
    ...
    for(QList<int>::const_iterator i=priorities.constBegin(); i!=priorities.constEnd(); i++) {
        ...
        for(QList<Entity*>::const_iterator ie=ents.constBegin(); ie!=ents.constEnd(); ie++) {
            (*ie)->setLoopFrequency(GlobalConstants::threadLoopFrequency());
            (*ie)->start();
        }
    }
}
...
void World::loop() {
    ...
    _opTeam->updateAvailablePlayers();
