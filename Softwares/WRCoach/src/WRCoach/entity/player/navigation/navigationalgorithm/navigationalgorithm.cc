...
void NavigationAlgorithm::generatePath() {
    ...
    while(WR::Utils::distance(currPos, _goalPos)>NAVALG_PATHRESOLUTION && timer.timemsec()<=NAVALG_MAXTIME) {
        ...
        float dx = NAVALG_PATHRESOLUTION*cos(direction.value());
        float dy = NAVALG_PATHRESOLUTION*sin(direction.value());
