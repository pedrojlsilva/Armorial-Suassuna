...
void Strategy::initialize(SSLReferee *ref, WRTeam *ourTeam, WRTeam *opTeam, CoachUtils *utils) {
    _ref = ref;
    _ourTeam = ourTeam;
    _opTeam = opTeam;
    _utils = utils;
...
void Strategy::runStrategy(int gameState, SSLGameInfo::RefProcessedState refState) {
    ...
    if(strategyState!=NULL) {
        if(strategyState->isInitialized()==false)
            strategyState->initialize(_ourTeam, _opTeam, _utils, _dist, &_kickerId, &_lastState, _ref);
