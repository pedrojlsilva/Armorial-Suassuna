...
void StrategyState::initialize(WRTeam *ourTeam, WRTeam *opTeam, CoachUtils *utils, PlayersDistribution *dist, qint8 *kickerId, SSLGameInfo::RefProcessedState *lastState, SSLReferee *ref) {
    _ourTeam = ourTeam;
    _opTeam = opTeam;
    _utils = utils;
    _dist = dist;
...
void StrategyState::runStrategyState() {
    ...
    for(it = _playbookList.begin(); it != _playbookList.end(); it++) {
        ...
        if(playbook->isInitialized()==false)
            playbook->initialize(_ourTeam, _opTeam, _utils, _kickerId, _lastState, _ref);
